"""
End-to-end tests for the file-area picker's `[O]rder` command (design
doc, dogfood feature request) -- drives the real `browse_file_areas`/
`_browse_areas_in_category`/`pick_item`/`prompt_sort_change` chain,
same shape `tests/test_chat_flow_picker_sort.py` established for the
channel picker. `netbbs.net.file_flow` is lane-based (its own module
docstring), so this mirrors that file's harness, not
`tests/test_login_flow_board_picker_sort.py`'s synchronous-`db` one.
"""
...
from netbbs.files.areas import create_file_area, update_file_area
...
def _set_created_at(db, area_name, iso_timestamp):
    db.connection.execute("UPDATE file_areas SET created_at = ? WHERE name = ?", (iso_timestamp, area_name))
    db.connection.commit()


def test_picker_shows_the_current_sort_mode_by_default(db, lane, alice):
...
def test_order_command_choosing_just_this_time_does_not_persist(db, lane, alice):
    create_file_area(db, "apple", creator=alice)
    create_file_area(db, "zebra", creator=alice)
    _set_created_at(db, "apple", "2026-01-01T00:00:00.000000Z")
    _set_created_at(db, "zebra", "2026-01-02T00:00:00.000000Z")
...
def test_community_scoped_order_offers_a_whole_community_save_option(db, lane, alice):
    ...
    update_file_area(
        db, area, name="downloads", description=None, min_read_level=0, min_write_level=0,
